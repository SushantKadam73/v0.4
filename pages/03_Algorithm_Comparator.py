...
All runs are logged to JSONL with a shared batch_id.
"""
...
st.caption(
    "Run all algorithms on the same input — compare performance, entropy, and security side-by-side."
)
...
input_mode = st.selectbox(
    "Input Mode", ["Text Input", "File Upload", "Synthetic Test Data"]
)
input_data = b""
...
if st.button("▶ Run Comparison", type="primary", disabled=not input_data):
    ...
    aes_sizes = [
        s
        for s, enabled in [(128, test_128), (192, test_192), (256, test_256)]
        if enabled
    ]
    algos = [
        a
        for a, e in [
            ("aes_gcm", run_aes),
            ("chacha20_poly1305", run_chacha),
            ("hybrid", run_hybrid),
        ]
        if e
    ]
    sigs = [
        s for s, e in [(None, run_no_sig), ("RSA", run_rsa), ("ECDSA", run_ecdsa)] if e
    ]
    ...
    for i, combo in enumerate(combos):
        ...
        row = {
            ...
            "Avalanche (%)": round(result.avalanche.bit_change_pct, 2)
            if result.avalanche
            else None,
        ...
        if not result.error:
            build_and_log_row(
                config, input_name, input_data, input_name + ".enc", result, bid, i + 1
            )
    ...
    prog.empty()
    st.success(
        f"✅ Comparison complete — {len(combos)} combinations tested | batch_id: `{bid}`"
    )
    ...
    with tab3:
        fig_e = px.bar(
            ...
            x="Label",
            y="Final Entropy",
            title="Final Ciphertext Entropy",
            color="Algorithm",
            barmode="group",
            range_y=[7.5, 8.1],
        )
    ...
    with tab4:
        ...
        if not ava_df.empty:
            fig_av = px.bar(
                ...
                x="Label",
                y="Avalanche (%)",
                title="Avalanche Effect (ideal = 50%)",
                color="Algorithm",
                barmode="group",
            )
            fig_av.add_hline(
            ...
            fig_av.add_hrect(
                ...
                fillcolor="green",
                opacity=0.1,
                annotation_text="Good range (40-60%)",
            )
    ...
    with tab6:
        ...
        available = [
            m for m in metrics if m in ok_df.columns and ok_df[m].notna().any()
        ]
    ...
    st.download_button(
        ...
        csv_bytes,
        file_name=f"comparison_{bid}.csv",
        mime="text/csv",
    )
